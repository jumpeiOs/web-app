...
export default function Contents() {
  ...
  let priceData = ["jp", "com", "uk", "es", "fr"];

  const omittedContent = (text: string, textLength: number) => {
  ...
  // const sortByPriceHandler = (priceData: any) => {
  // for (let key in data) {
  //   // Object.keys(data).forEach(function (key) {
  //   if (data.hasOwnProperty(key)) {
  //     let price = [
  //       data.key.price_jp + data.key.extra_fee_jp,
  //       data.key.price_com + data.key.extra_fee_com,
  //       data.key.price_fr + data.key.extra_fee_fr,
  //       data.key.price_es + data.key.extra_fee_es,
  //       data.key.price_uk + data.key.extra_fee_uk,
  //     ];
  //     price.sort(([, a], [, b]) => a - b);
  //   }
  // }
  // for (let value of searchedData) {
  //   if (value.asin_jp in Object.keys(data)) {
  //     data.value.asin_jp;
  //   }
  // }
              ...
              <CardContent>
                ...
                <Grid container sx={{ alignItems: "center" }}>
                    ...
                    >
                      <ImageListItem key={item.img_path}>
                        <Box
                          ...
                          srcSet={`${item.img_path}?w=164&h=164&fit=crop&auto=format&dpr=2 2x`}
                          alt={omittedContent(item.title_jp, 20)}
                          loading="lazy"
                        />
                      </ImageListItem>
                    </ImageList>
                  ...
                  >
                    ...
                    >
                      <Box>{omittedContent(item.category, 10)}</Box>
                      <Box>{omittedContent(item.brand, 10)}</Box>
                    </Box>
                    <Typography
                      sx={{
                        fontSize: 18,
                    ...
                    >
                      {omittedContent(item.title_jp, 35)}
                    </Typography>
                  </Grid>
                </Grid>
                <ul>
